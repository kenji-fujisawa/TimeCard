...
@Composable
private fun CalendarRecordScreenPreview() {
    TimeCardClientAndroidTheme {
        ...
        CalendarRecordScreen(CalendarUiState.CalendarRecord(date = formatter.parse("2025-12-04 00:00:00") ?: Date(), records = listOf(record1, record2)))
    }
}

@Preview(showBackground = true)
@Composable
private fun CalendarRecordScreenPreviewSingleLine() {
    TimeCardClientAndroidTheme {
        val formatter = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault())
        ...
        CalendarRecordScreen(CalendarUiState.CalendarRecord(date = formatter.parse("2025-12-07 00:00:00") ?: Date(), records = listOf(record1)))
    }
}
